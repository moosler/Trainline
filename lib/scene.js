...
class Scenewindow {
  ...
  addLevels(number) {
    ...
    for (let i = 0; i < 24; i++) {
      ...
      if (number > i) {
        let button2 = new PIXI.Sprite(textureId["blank_gold.png"]);
        button.texture = button2.texture;
      }
      button.width = 50;
      button.height = 50;
      button.position.set(
  ...
  addButtonGroup(buttonPos) {
    let play_button = new PIXI.Sprite(textureId["play.png"]);
    play_button.width = 50;
    play_button.height = 50;
    play_button.position.set(
      (_WIDTH - this.sprite.width) / 2 + buttonPos.x,
      this.sprite.height + buttonPos.y
    );
    play_button.interactive = true;
    play_button.buttonMode = true;
    play_button.mouseover = function(mouseData) {
      this.tint = "0xcccc00";
    };
