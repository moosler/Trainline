  ...
  recalcLevels(l_index, player_selection) {
    ...
    for (let i = 0; i < textures.length; i++) {
      ...
      el.mousedown = function () {
        ...
        game.scenes.recalcLevels(
          game.player.unlockedLevels,
          game.player.selectedLevel
        );
      };
  ...
  addButtonGroup(buttonPos) {
    ...
    for (let i = 0; i < buttons.length; i++) {
      ...
      button.mouseover = function (mouseData) {
        this.tint = "0xcccc00";
        let tp = game.screen.tooltip;
        tp.container.visible = true;

        /**@todo refactor with callbacks */
        //LevelEditor
        if (i == 0) {
          tp.showText("Custom Level Editor", this.x, this.y);
        }
        // Map Generator
        else if (i == 1) {
          tp.showText("Procedural Map Generator", this.x, this.y);
        }
        // Play Button
        else if (i == 2) {
          tp.showText("Play slected Level", this.x, this.y);
        }
      };
      button.mouseout = function () {
        this.tint = "0xffffff";
        game.screen.tooltip.container.visible = false;
      };
      button.mousedown = function () {
        ...
        // Map Generator
          ...
          game.generator = new MapGenerator(0, true);
        }
        // Play Button
        else if (i == 2) {
