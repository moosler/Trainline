...
class MapGenerator {
  constructor(seed = 55555, showInputMenu = false) {
    this.seed = seed;
    this.rand = {};
    this.route = [];
    this.stations = [];
    ...
    this.showInputMenu = showInputMenu;
    this.init();
  }
  init() {
    game.reset();
    ...
    if (this.showInputMenu) {
      game.screen.createInput();
      game.screen.inputMenu.visible = true;
      this.startGeneration(false);
    } else {
      game.screen.inputMenu.visible = false;
      this.startGeneration(true);
    }
  }

  startGeneration(play = false) {
    this.generateMap();
    // this.map.saveMap();
  ...
  getOrientation(tile) {
    ...
    if (tile.x >= game.grid.x - 1) return 3;
