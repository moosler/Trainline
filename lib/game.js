class Game {
  constructor(width, height, app, config = {}) {
    ...
    this.timetable = {};
    this.generator = false; //Mape Generator
  ...
  setStatus(status = null) {
    ...
    //Select Level Menu
      ...
      this.scenes.recalcLevels(this.levelIndex, this.player.selectedLevel);
      this.setAllNonVisible();
  ...
  nextLevel(number = false) {
    ...
    number === false ? this.levelIndex++ : (this.levelIndex = number);
    this.reset();
    let current_map = "maps/" + MAPS[this.levelIndex] + ".json";
