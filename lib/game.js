class Game {
  ...
  loadTrack() {
    ...
    let dimensions = {width: this.grid.xWidth, height: this.grid.yHeight}
    this.track.loadRoute(this.grid.tiles, dimensions);
  }
  addTrains() {
    this.addTrain(0, 0);
  }
  addTrain(x, y) {
    ...
    let train = new Train({ x: x, y: y},{ x: this.grid.xWidth / 4, y: this.grid.yHeight}, route);
