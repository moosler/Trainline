...
class Rail {
   ...
   * @param {*} width 
   * @param {*} height 
   * @param {*} type type[0] = 0 = down; 1= up
   *                 type[1] = 0 = right; 1 = left
   */
  setCurve(width, height, type){
    /**clockwise start at clock position 3 */

    // type 1,2
    // this.pos = {x: this.mid.x, y: this.mid.y+ height / 2};
    // this.control = {x1:  this.mid.x, y1: this.mid.y+(height/4) ,x2: this.mid.x-(width/4), y2: this.mid.y};
    // this.end = {x: this.mid.x-(width/2), y: this.mid.y};
