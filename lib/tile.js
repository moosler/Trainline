class Tile {
  ...
  getNext(tile) {
    let next = this.calcNext(tile);
    if (next) {
      if (tile instanceof Train && this.type != 6) {
        //is undefined if train leaves the grid
        return this.calcForTrain(next, tile.direction, tile);
      } else {
        return this.calcforRoute(next, tile.direction);
      }
    }
    return false;
  }
  ...
  setTree() {
    let objects = ["tree_small.png", "palm.png", "stump.png", "cactus.png"];
    let objectIndex = Math.floor(Math.random() * objects.length);
    let objectName = objects[objectIndex];
    this.graphic = new PIXI.Sprite(textureId[objectName]);
    this.graphic.width = this.width - 5;
    this.graphic.height = this.height - 5;
