class Train {
  constructor(pos, dim, track, start, color = "0xe74c3c") {
  ...
  init() {
    ...
    this.direction = this.tile.calcStartDirection();
  }
  /**@deprecated */
  getTile() {
    if (this.tileIndex < this.track.route.length) {
      let tile = this.track.route[this.tileIndex];
      this.tileIndex++;
      this.tile = tile;
    }
    this.tile = false;
  ...
  setTile() {
    if (this.passedRails !== 1) {
      ...
      let tile = this.tile.getNext(this.direction);
      this.setDirection();
      this.tile = tile;
    }
  }
  ...
  step() {
    ...
    if (tick == 0) {
      ...
      this.curLine = this.tile.getLine();
    }
    this.update();
    return true;
  }
